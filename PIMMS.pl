...
my $VERSION = "PIMMS pipeline version 1.92 March 2015";
...
If no aligner is provided the default recommended mapper BWA with options mem -t 2 will be be used.
...
my @motifs =  split '\t', $commands[0]; 
my $motif_1 = $motifs[0];
chomp $motif_1;
$motif_1 =~ s/ //g;

my $motif_2 = "~~~~~~~~~~~~"; # nonsense holder that will not be found in sequence 
my $single_motif = "TRUE";
...
my $motif_2_rev_comp =  "~~~~~~~~~~~~~~"; # holding regex that will not match if only a single motif is requested.
if ($single_motif eq "FALSE")
	{
	my $bar = reverse $motif_2;
	$bar =~ tr/ACGTacgt/TGCAtgca/;
	$motif_2_rev_comp = $bar;
	}
